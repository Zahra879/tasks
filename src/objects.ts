...
export function isCorrect(question: Question, answer: string): boolean {
    const answer1 = answer.trim().toLowerCase();
    const expecte = question.expected.trim().toLowerCase();

    //const boolin = (answer: string): boolean => answer === answer;
    return answer1 === expecte;
}
...
export function toShortForm(question: Question): string {
    const str = question.id + ": " + question.name.slice(0, 10);
    return str;
}
